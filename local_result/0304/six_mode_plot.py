...
def normalize_timelines(req_all: pd.DataFrame, vram_all: pd.DataFrame, evt_all: pd.DataFrame, hit_all: pd.DataFrame):
    ...
    for mode in modes:
        ...
        if not h.empty: candidates.append(float(h["ts"].min()))
...
def main():
    ...
    for mode in order:
        ...
        if files[mode]["vram_kind"]=="raw":
            vram, evt, hit = parse_vram_raw_log(files[mode]["vram"], mode)
        else:
            vram, evt, hit = parse_vram_json(files[mode]["vram"], mode)
    ...
    summary_rows=[]
    for mode in order:
        req_df=req_all[req_all["mode"]==mode].sort_values("t_finish")
        hit_df=hit_all[hit_all["mode"]==mode].sort_values("t")
