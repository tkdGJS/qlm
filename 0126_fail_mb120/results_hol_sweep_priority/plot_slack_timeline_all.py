...
def classify_long_short(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "out_tok" in df.columns:
        ...
        # fallback: execution_time median
        df["is_long"] = df["execution_time"] > pd.to_numeric(df["execution_time"], errors="coerce").median()
    return df
...
def plot_one(csv_path: str, out_dir: str, maxN: int = 200):
    ...
    required = ["orig_slo", "insertion_time", "wait_time", "execution_time", "finished_time"]
    for c in required:
