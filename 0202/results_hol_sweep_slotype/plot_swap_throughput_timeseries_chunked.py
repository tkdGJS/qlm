...
def parse_meta(path: str) -> Tuple[Optional[float], Optional[float], Optional[int], Optional[int]]:
    m = FNAME_RE.match(os.path.basename(path))
    if not m:
        return None, None, None, None
    return float(m.group("sleep")), float(m.group("pushint")), int(m.group("mb")), int(m.group("chunked"))
...
def process_file(path: str, outdir: str, args: argparse.Namespace):
    ...
    sleep, pushint, mb, chunked = parse_meta(path)
    title_prefix = ""
    if mb is not None:
...
def main(args: argparse.Namespace) -> None:
    ...
    for p in files:
        sleep, pushint, mb, chunked = parse_meta(p)
        if sleep is None or pushint is None:
            gname = os.path.join("chunked_unknown", "misc")
        else:
            gname = os.path.join(f"chunked_{chunked}", f"sleep_{sleep}_pushint_{pushint}")
