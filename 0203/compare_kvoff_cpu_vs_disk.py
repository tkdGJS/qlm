...
def ecdf(ax, x: np.ndarray, label: str):
...
def plot_ecdf_pair(df_cpu: pd.DataFrame, df_disk: pd.DataFrame,
    ...
    for i, slo in enumerate([0, 1]):
        ...
        ecdf(ax, cpu_vals,  "CPU-only")
        ecdf(ax, disk_vals, "CPU+storage")
        ax.set_title(f"slo_type={slo}")
...
def plot_throughput_pair(df_cpu: pd.DataFrame, df_disk: pd.DataFrame, outpath: Path):
    ...
    ax.plot(tp_cpu.index, tp_cpu.values, label="CPU-only")
    ax.plot(tp_disk.index, tp_disk.values, label="CPU+storage")
...
def plot_cache_latency_overlay(df: pd.DataFrame, outpath: Path,
    ...
    title = f"{mode_label} overlay (slo_type={slo_type}): cache usage + latency + events"
    ax1.set_title(title)
...
def plot_delta_time_dist(df_cpu: pd.DataFrame, df_disk: pd.DataFrame,
    ...
    # ECDF
    ecdf(ax, cpu,  "CPU-only")
    ecdf(ax, disk, "CPU+storage")
    ax.set_xlabel(xlabel)
    ax.set_ylabel("ECDF")
    ax.set_title(title)
    ax.grid(True, alpha=0.25)
...
def main():
    ...
    for (sleep, pushint, mb, chunked) in keys:
        ...
        df_cpu = prep_df(df_cpu)
        df_disk = prep_df(df_disk)
        ...
        plot_throughput_pair(df_cpu, df_disk, outp)
        ...
        for mode_label, d in [("CPU-only", df_cpu), ("CPU+storage", df_disk)]:
            outp0 = pair_out / f"C_overlay_{mode_label}_slo0_TTFT.png"
            ...
            outp1 = pair_out / f"C_overlay_{mode_label}_slo1_TTLT.png"
            plot_cache_latency_overlay(
