...
# --- Axis labels (user-requested wording) ---
YLABEL = {
...
def plot_counts_vs_mb(df: pd.DataFrame, outdir: str) -> None:
    plot_lines(
        df,
        x="mb",
        ys=[("run_cnt_mean", "run_cnt (mean)"), ("wait_cnt_mean", "wait_cnt (mean)"), ("swap_cnt_mean", "swap_cnt (mean)")],
        title="Running/Waiting/Swapped Request Counts vs mb",
        ylabel="count (mean over samples)",
...
def plot_distributions(df_by_mb: Dict[int, pd.DataFrame], outdir: str) -> None:
    ...
    for metric, title, xlabel in [
        ("kv", "KV Distribution (ECDF)", "kv (unit from log)"),
        ("swap_cnt", "swap_cnt Distribution (ECDF)", "swap_cnt"),
        ("vram_used_gib", "VRAM Used Distribution (ECDF)", "GiB"),
    ]:
...
def plot_time_series_like(df_by_mb: Dict[int, pd.DataFrame], outdir: str) -> None:
    ...
    metrics = ["kv", "run_cnt", "wait_cnt", "swap_cnt", "vram_used_gib"]
    ...
    for metric in metrics:
        ...
        for mb in sorted(df_by_mb.keys()):
            ...
            if m.sum() < 10:
                continue
            # NOTE: user-requested: do NOT normalize time; keep original time axis
            tn = t[m]
            vn = v[m]
            # downsample to reduce clutter
            idx = np.linspace(0, len(tn) - 1, num=min(500, len(tn))).astype(int)
            # User-requested: swap_cnt should always be red.
            if metric == "swap_cnt":
                plt.plot(tn[idx], vn[idx], label=f"mb{mb}", alpha=0.8, color="red")
            else:
        ...
        if plotted:
            plt.xlabel("time (s)")
            plt.ylabel(YLABEL.get(metric, metric))
            plt.title(f"{YLABEL.get(metric, metric)} over time (per-request samples)")
            plt.legend(ncol=2, fontsize=9)
...
def plot_dual_timeseries_per_mb(df_by_mb: Dict[int, pd.DataFrame], outdir: str) -> None:
    """Per-mb dual-axis time-series plots.

    Goals (user-requested):
    - Do NOT normalize time.
    - Ensure run_cnt trace shape is identical between the run-only plot and
      the run+swap plot by reusing the exact same time/value sampling path.
    - swap_cnt must be red.
    - Add an additional plot to view kv and swap_cnt together.
    """

    # choose a time column once (consistent with plot_time_series_like)
    for tcol in ["finished_time", "insertion_time"]:
        ok = any(tcol in df.columns for df in df_by_mb.values())
        if ok:
            time_col = tcol
    ...
    def downsample_idx(n: int) -> np.ndarray:
        if n <= 0:
            return np.array([], dtype=int)
        return np.linspace(0, n - 1, num=min(500, n)).astype(int)
    ...
    for mb in sorted(df_by_mb.keys()):
        ...
        # ---------- (1) run_cnt + swap_cnt ----------
        if "run_cnt" in df.columns and "swap_cnt" in df.columns:
            run_raw = pd.to_numeric(df["run_cnt"], errors="coerce").to_numpy(dtype=float)
            swap_raw = pd.to_numeric(df["swap_cnt"], errors="coerce").to_numpy(dtype=float)

            # IMPORTANT: use the same mask as the run-only plot would use
            m = np.isfinite(t_raw) & np.isfinite(run_raw)
            if m.sum() >= 10:
                t = t_raw[m]
                run = run_raw[m]
                swap = swap_raw[m]  # keep aligned; may include NaNs
                idx = downsample_idx(len(t))

                fig, ax1 = plt.subplots(figsize=(10.5, 4.0))
                l1 = ax1.plot(t[idx], run[idx], label=YLABEL.get("run_cnt", "run_cnt"), alpha=0.85)
                ax1.set_xlabel("time (s)")
                ax1.set_ylabel(YLABEL.get("run_cnt", "run_cnt"))
                ...
                l2 = ax2.plot(
                    ...
                    color="red",
                    linestyle="--",
                    label=YLABEL.get("swap_cnt", "swap_cnt"),
                    alpha=0.85,
                )
                ax2.set_ylabel(YLABEL.get("swap_cnt", "swap_cnt"))
                ...
                savefig(os.path.join(outdir, f"timeseries_run_swap_mb{mb}.png"))

        # ---------- (2) kv + swap_cnt ----------
        if "kv" in df.columns and "swap_cnt" in df.columns:
            kv_raw = pd.to_numeric(df["kv"], errors="coerce").to_numpy(dtype=float)
            swap_raw = pd.to_numeric(df["swap_cnt"], errors="coerce").to_numpy(dtype=float)

            # align by kv-valid samples (so KV curve itself is not distorted)
            m = np.isfinite(t_raw) & np.isfinite(kv_raw)
            if m.sum() >= 10:
                t = t_raw[m]
                kv = kv_raw[m]
                swap = swap_raw[m]
                ...
                fig, ax1 = plt.subplots(figsize=(10.5, 4.0))
                l1 = ax1.plot(t[idx], kv[idx], label=YLABEL.get("kv", "kv"), alpha=0.85)
                ax1.set_xlabel("time (s)")
                ax1.set_ylabel(YLABEL.get("kv", "kv"))
                ...
                ax1.set_title(f"kv and swap_cnt over time (mb{mb})")
                lines = (l1 + l2)
                ax1.legend(lines, [ln.get_label() for ln in lines], loc="upper right", fontsize=9)
                savefig(os.path.join(outdir, f"timeseries_kv_swap_mb{mb}.png"))
...
def main(args: argparse.Namespace) -> None:
    ...
    for (sleep, pushint), group in grouped.items():
        ...
        plot_dual_timeseries_per_mb(df_by_mb, sub)
