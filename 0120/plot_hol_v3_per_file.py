...
def plot_one(csv_path: str, out_dir: str):
    ...
    for _, r in sub.iterrows():
        ...
        plt.plot([r["_d0"], r["_f0"]], [r["arr_idx"], r["arr_idx"]], color=color, linewidth=2, solid_capstyle="butt")
    plt.xlabel("Time since first arrival [s]")
    plt.ylabel("Arrival order index")
    plt.title(f"Timeline : {title_suffix}")
    ...
    if "orig_slo" in df.columns:
        ...
        # proxy for legend
        plt.plot([], [], color="0.7", linewidth=1, label="late wait (after SLO deadline)")
