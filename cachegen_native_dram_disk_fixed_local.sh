...
PROJECT_ROOT="${PROJECT_ROOT:-/home/yuhwa2323/noslab_gpu}"
...
if [[ -z "$PY" || ! -x "$PY" ]]; then
  echo "ERROR: Python executable not found or not executable: ${PY:-<empty>}" >&2
  exit 1
fi
...
LMCACHE_CONFIG_FILES=(
  "${PROJECT_ROOT}/local_lmcache_native_dram.yaml"
  "${PROJECT_ROOT}/local_lmcache_native_disk.yaml"
  "${PROJECT_ROOT}/local_lmcache_native_dram_disk.yaml"
  "${PROJECT_ROOT}/local_lmcache_cachegen_dram.yaml"
  "${PROJECT_ROOT}/local_lmcache_cachegen_disk.yaml"
  "${PROJECT_ROOT}/local_lmcache_cachegen_dram_disk.yaml"
)
