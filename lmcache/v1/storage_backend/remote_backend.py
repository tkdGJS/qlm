...
class RemoteBackend(StorageBackendInterface):
    ...
    def _maybe_vram_log(self, event: str) -> None:
        if os.environ.get("LMCACHE_VRAM_LOG", "0") != "1":
            return
        ts = time.time()
        msg = f"[LMCACHE_VRAM][RemoteBackend] {ts:.6f} {event}"
    ...
    def _get_thread_local_serde(self):
        if getattr(self._serde_tls, "serializer", None) is None or getattr(
            ...
            if os.environ.get("LMCACHE_SERDE_DEBUG", "0") == "1":
                ts = time.time()
                msg = (
                    f"[SERDE_THREAD] {ts:.6f} RemoteBackend "
    ...
    def batched_submit_put_task(
        ...
        if self.connection.support_batched_put():
            ...
            try:
                for memory_obj in memory_objs:
                    compressed_memory_objs.append(serializer.serialize(memory_obj))
            finally:
    ...
    def get_blocking(
        ...
        self._maybe_vram_log("deserialize")
        decompressed_memory_obj = deserializer.deserialize(memory_obj)
    ...
    def batched_get_blocking(
        ...
        for memory_obj in memory_objs:
            if memory_obj is None:
                ...
                _, deserializer = self._get_thread_local_serde()
                self._maybe_vram_log("deserialize")
                decompressed_memory_objs.append(deserializer.deserialize(memory_obj))
        if error_happened:
            self._interval_get_blocking_failed_count += 1
