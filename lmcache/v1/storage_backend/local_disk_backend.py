...
from lmcache.v1.memory_management import BytesBufferMemoryObj, MemoryFormat, MemoryObj
...
class LocalDiskBackend(StorageBackendInterface):
    def __init__(
        ...
        if config.extra_config is not None:
            try:
                disk_max_workers = int(
                    config.extra_config.get("local_disk_max_workers", disk_max_workers)
                )
            except Exception:
                disk_max_workers = 4
        if disk_max_workers < 1:
            disk_max_workers = 1
        self.disk_worker = LocalDiskWorker(loop, max_workers=disk_max_workers)
        ...
        if self._serde_enabled:
            logger.info(
                "LocalDiskBackend: Thread-local serde enabled: %s",
                self._serde_type,
            )
        else:
            logger.info("LocalDiskBackend: No serde configured, using raw bytes")

        # VRAM logging
        self.vram_log_enabled = os.environ.get("LMCACHE_VRAM_LOG", "0") == "1"
        result_dir = os.environ.get("LMCACHE_RESULT_DIR", "./result")
        self.vram_log_file = os.environ.get(
            "LMCACHE_VRAM_LOG_FILE",
            os.path.join(result_dir, "lmcache_vram.log"),
        )
        os.makedirs(os.path.dirname(self.vram_log_file), exist_ok=True)
    ...
    def _get_thread_local_serde(self):
        if not self._serde_enabled:
            return None, None
        if getattr(self._serde_tls, "serializer", None) is None or getattr(
            self._serde_tls, "deserializer", None
        ) is None:
            from lmcache.v1.storage_backend.naive_serde import CreateSerde

            assert self._serde_metadata is not None
            assert self._serde_type is not None
            s, d = CreateSerde(self._serde_type, self._serde_metadata, self._serde_config)
            self._serde_tls.serializer = s
            self._serde_tls.deserializer = d

            if os.environ.get("LMCACHE_SERDE_DEBUG", "0") == "1":
                ts = time.time()
                msg = (
                    f"[SERDE_THREAD] {ts:.6f} LocalDiskBackend "
                    f"thread={threading.current_thread().name} "
                    f"serializer_id={id(s)} deserializer_id={id(d)}"
                )
    ...
    def async_save_bytes_to_disk(
        ...
        if serializer is not None:
            compressed_obj = serializer.serialize(memory_obj)
        ...
        if self.vram_log_enabled:
            current_time = time.time()
            log_msg = f"[LMCACHE_VRAM][LocalDiskBackend] {current_time:.6f} serialize"
            print(log_msg, flush=True)
            with open(self.vram_log_file, "a") as f:
                f.write(log_msg + "\n")
        ...
        for idx, (path, key, mem_obj) in enumerate(
            ...
            if deserializer is not None:
                ...
                if self.vram_log_enabled:
                    current_time = time.time()
                    log_msg = (
                        f"[LMCACHE_VRAM][LocalDiskBackend] {current_time:.6f} deserialize"
                    )
                    print(log_msg, flush=True)
                    with open(self.vram_log_file, "a") as f:
                        f.write(log_msg + "\n")

                compressed_obj = BytesBufferMemoryObj(bs)
                decoded_obj = deserializer.deserialize(compressed_obj)
                # decoded_obj has ref_count=-1 (CacheGen HACK, GPU tensor, unmanaged).
                # Release the pre-allocated CPU mem_obj and return decoded_obj
                # (GPU tensor) directly.
                # ref_count_down() on decoded_obj by the caller is a no-op
                # (guarded in TensorMemoryObj.ref_count_down for ref<0).
                mem_obj.ref_count_down()  # release CPU allocation (ref=1→0→freed)
                decoded_obj.metadata.cached_positions = self.dict[key].cached_positions
                memory_objs[idx] = decoded_obj
            else:
                if self.vram_log_enabled:
                    current_time = time.time()
                    log_msg = (
    ...
    def load_bytes_from_disk(
        ...
        if deserializer is not None:
            ...
            decoded_obj = deserializer.deserialize(compressed_obj)
            # decoded_obj has ref_count=-1 (CacheGen HACK, GPU tensor, unmanaged).
            # Return it directly; ref_count_down() on the caller side is a
            # no-op (guarded in TensorMemoryObj.ref_count_down for ref<0).
            decoded_obj.metadata.cached_positions = self.dict[key].cached_positions
            return decoded_obj

        if self.vram_log_enabled:
            current_time = time.time()
            log_msg = f"[LMCACHE_VRAM][LocalDiskBackend] {current_time:.6f} deserialize"
            print(log_msg, flush=True)
            with open(self.vram_log_file, "a") as f:
                f.write(log_msg + "\n")
