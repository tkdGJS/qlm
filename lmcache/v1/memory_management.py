    ...
    def ref_count_down(self):
        with self.lock:
            if self.meta.ref_count < 0:
                # ref_count=-1 is a sentinel for unmanaged tensors
                # (e.g. CacheGen decoded GPU tensors whose lifecycle is
                # managed by Python GC, not the LMCache allocator).
                # Skip decrement entirely to avoid double-free warnings.
                return
            self.meta.ref_count -= 1
            if (
