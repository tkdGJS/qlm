...
def encode_function(
    ...
    """
    import os
    log_vram = os.environ.get("LMCACHE_VRAM_LOG_INTERNAL", "0") == "1"
    result_dir = os.environ.get("LMCACHE_RESULT_DIR", "./result")
    vram_log_file = os.environ.get(
        "LMCACHE_VRAM_LOG_FILE",
        os.path.join(result_dir, "lmcache_vram.log"),
    )
    os.makedirs(os.path.dirname(vram_log_file), exist_ok=True)
    ...
    def log_step(name, mem_before):
        if log_vram:
            ...
            log_msg = f"[LMCACHE_VRAM] {current_time:.6f} {name}"
    ...
    for i in range(0, chunk_size, CGBasics.CACHEGEN_GPU_MAX_TOKENS_PER_CHUNK):
        ...
        if log_vram and i == 0:
            current_time = time.time()
            log_msg = f"[LMCACHE_VRAM] {current_time:.6f} 08_encode_ntokens_chunk"
            print(log_msg, flush=True)
            with open(vram_log_file, "a") as f:
                f.write(log_msg + "\n")
                f.flush()
    mem = log_step("07_encode_ntokens_loop", mem)
    
    # Final result
    if log_vram:
        current_time = time.time()
        log_msg = f"[LMCACHE_VRAM] {current_time:.6f} encode_function"
        print(log_msg, flush=True)
        with open(vram_log_file, "a") as f:
            f.write(log_msg + "\n")
            f.flush()
