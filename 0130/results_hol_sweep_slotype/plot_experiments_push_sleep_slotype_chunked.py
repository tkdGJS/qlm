...
"""
plot_experiments_push_sleep_slotype_chunked.py

기존: pushint별로 (sleep 라인 여러개) 그래프 생성
추가: 파일명에 포함된 chunked_{0|1}, mb{N} 기준으로 먼저 분리해서
     outdir/chunked_{c}/mb{mb}/push_{push}/... 형태로 저장.
"""
...
def build_color_map_for_sleep(sleeps: List[float]) -> Dict[float, str]:
    ...
    out: Dict[float, str] = {}
    def k(x: float) -> float:
        return SLEEP_ORDER.index(x) if x in SLEEP_ORDER else x
    for i, s in enumerate(sorted(sleeps, key=k)):
        out[s] = colors[i % len(colors)]
    return out

def load_csvs(paths: List[Path]) -> Dict[float, Dict[float, pd.DataFrame]]:
    """Return Dict[sleep][push] = df (mb/chunked는 main에서 분리)."""
...
def plot_multi_line_by_sleep(
    ...
    for sleep, df in sorted(data.items(), key=lambda kv: k(kv[0])):
...
def main():
    ...
    for (ch_label, mb_label), gpaths in sorted(groups.items()):
        ...
        data = load_csvs(gpaths)  # Dict[sleep][push] = df
        if not data:
        ...
        for push in all_pushes:
            push_str = str(push)
            push_dir = base_out / f"push_{push_str}"
            ...
            plot_multi_line_by_sleep(
                ...
                title=f"(1) Success Rate (%) vs CSV Row Index | {ch_label}/{mb_label} push={push_str}",
            ...
            plot_multi_line_by_sleep(
                push_data, colors,
                ycol="ttlt",
                title=f"(2) TTLT vs CSV Row Index | {ch_label}/{mb_label} push={push_str}",
                ylabel="ttlt (s)",
            ...
            plot_multi_line_by_sleep(
                push_data, colors,
                ycol="wait_time",
                title=f"(4) Wait Time vs CSV Row Index | {ch_label}/{mb_label} push={push_str}",
                ylabel="wait_time (s)",
            ...
            plot_multi_line_by_sleep(
                push_data, colors,
                ycol="violation",
                title=f"(5) Violation vs CSV Row Index | {ch_label}/{mb_label} push={push_str}",
                ylabel="violation",
            ...
            plot_multi_line_by_sleep(
                push_data, colors,
                ycol="orig_slo",
                title=f"(6) SLO vs CSV Row Index | {ch_label}/{mb_label} push={push_str}",
                ylabel="SLO (sec)",
