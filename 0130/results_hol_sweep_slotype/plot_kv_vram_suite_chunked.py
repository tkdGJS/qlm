...
FNAME_RE = re.compile(
    r".*sleep_(?P<sleep>[\d\.]+)_pushint_(?P<pushint>[\d\.]+)_mb(?P<mb>\d+)_chunked_(?P<chunked>\d+)_.*\.csv$"
)
...
def main(args: argparse.Namespace) -> None:
    ...
    grouped: Dict[Tuple[int, float, float], List[RunMeta]] = {}
    for m in metas:
        grouped.setdefault((m.chunked, m.sleep, m.pushint), []).append(m)

    for (chunked, sleep, pushint), group in grouped.items():
        group = sorted(group, key=lambda x: x.mb)
        print(f"\n=== Group chunked={chunked}, sleep={sleep}, pushint={pushint} ({len(group)} files) ===")
        ...
        sub = os.path.join(args.outdir, f"chunked_{chunked}", f"sleep_{sleep}_pushint_{pushint}")
        ensure_dir(sub)
