...
async def basic_test():
    ...
    SLO_TBT_INTERACTIVE_S = float(os.environ.get("SLO_TBT_INTERACTIVE_S", "1"))
    SLO_TBT_BATCH_S = float(os.environ.get("SLO_TBT_BATCH_S", "200"))
...
#    long_pool = [x for x in dataset2_candidates if x["base_tok_len"] >= LONG_MIN_BASE_PROMPT_TOKENS]
    ...
    def sample_slo(slo_type: int) -> float:
        if slo_type == 0:
            return SLO_TTFT_INTERACTIVE_S
        return SLO_TBT_BATCH_S
