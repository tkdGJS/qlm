...
def _usage() -> None:
    ...
    print("  - If omitted, mode is inferred from $MODE or $LMCACHE_CONFIG_FILE.")


def _infer_mode_from_env() -> str:
    mode = (os.environ.get("MODE", "") or "").strip().lower()
    if mode in ("native", "cachegen"):
        return mode

    cfg = (os.environ.get("LMCACHE_CONFIG_FILE", "") or "").strip()
...
async def basic_test() -> None:
    ...
    push_interval_raw = (os.environ.get("PUSH_INTERVAL_S") or os.environ.get("SLEEPS") or "0.1")
    PUSH_INTERVAL_S = float(push_interval_raw)
    ...
    MAX_TOKENS = int(max_tokens_raw)  # 0 = no limit (vLLM model default)
    ...
    print(
        ...
        f"LMCACHE_RESULT_DIR={os.environ.get('LMCACHE_RESULT_DIR', '')}",
    ...
    print(
        ...
        f"VLLM_EXTRA_ARGS={os.environ.get('VLLM_EXTRA_ARGS', '')}",
